...
tags:
  - pro
subtitle: ''
summary: 'The K-fold cross validation allows to quantify the performance of a forecasting model. This article explains the implementation of this procedure for timeseies in the context of a VAR model. It makes use of the kfoldcv4ts package that I created.'
authors: []
...
In this article, I provide a little summary of k-fold cross validation and provide an application in R using a dataset consisting of several US macroeconomic variables (from atStock2002). For illustration purpose, I evaluate the relative performance of several competing VAR models for predicting quarterly US GDP using a k-fold cross validation approach adapted to timeseries. I also provide the commands to install my package `kfoldcv4ts` that allows to perform the k-fold cross validation for timeseries (function `accuracy.kfold`). The package makes use of several existing packages, most notably the excellent `forecast` and `vars` package.


The function used in this article (`accuracy.kfold`) can be downloaded as a package from my GitHub repo using the following command:
...
For the procedure to work on your computer, you need to install/load the following packages: `dplyr`, `vars`, `forecast`, and `AER`. Most of these packages are also required for installing the `kfoldcv4ts` package.
...
K-fold cross validation is precisely a way to split the data in a training and test dataset. The easies way to understand k-fold cross validation is certainty to look at a graphical representation:
...
We use macroeconomic time series from the `AER` package. The dataset (`USMacro`) is from the atStock2007. It consists of 7 time series from 1957 to 2005 available at the quarterly frequency. The following commands load the data in R:
```{r, warning = FALSE, include=FALSE}
library('AER')
```
...
As an exercise, we decide to estimate VAR consisting of the first 6 variables:
```{r, echo = TRUE}
...
We now use our function to perform a k-fold cross validation. Information about the function can found on [GitHub](link_to_github) ([source code of the function](link)) or by downloading the package. In a few w beords, the function splits the data in k training and test folds. It then estimates a VAR on each fold and returns various out-of-sample measures for each fold. The last element of the list is the k-th fit of the model and uses the whole dataset (minus the `n_ahead` last observations that are used as a test dataset) for the training of the model.
