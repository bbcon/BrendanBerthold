...
#' @export f.accuracy.kfold(df.VAR, k=5,n_ahead,lags = 2,var_index = 1)
#'
#' @examples f.accuracy.kfold(df.VAR, k=5,n_ahead,lags = 2,var_index = 1)
f.accuracy.kfold = function(df.VAR, k=5,n_ahead,lags,var_index = 1){
  ...
  for(i in 1:k){
    ...
    list.predict[[i]] = predict(list.VAR[[i]], n.ahead = n_ahead)
    list.fitted[[i]] = fitted(list.VAR[[i]])
  }
